...
<hr>
	<a href="<?php echo site_url('payroll_extra/insert_bonus');?>" class="button"><span class="add">Внес</span></a>
<div class="filters"> 
    <?php echo form_open('payroll_extra/search');?>
...
<?php if (isset($results) AND is_array($results) AND count($results) > 0):?>
	<tr>
		...
		<?php foreach ($columns as $col_name => $col_display):?>
			<th <?php if($sort_by==$col_name) echo "class=$sort_order";?>>
    			<?php echo anchor("payroll_extra/index/{$query_id}/{$col_name}/".(($sort_order=='desc' && $sort_by==$col_name)?'asc':'desc'),$col_display);?>
			</th>
    	<?php endforeach;?>
		<th>&nbsp;</th>
	</tr>
	<?php foreach($results as $row):?>
		<tr>
			<td class="code" align="center"><?php echo anchor("payroll_extra/view/{$row->id}/1",'&nbsp;','class="view_icon"');?></td>
